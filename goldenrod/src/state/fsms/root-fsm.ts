...
import { FsmConfigOption, InteractionState, RootInteractionState, fsmConfig, type RootStateEnum } from "./types";
import { keyboardInput, liftEvent, pressEvent } from "src/components/keyboard-ctrl/keyboard-input";
...
const baseConfig = Object.values(RootInteractionState).reduce((acc, stateName) => {
  let stateConfig = {};
...
export const rootFsm = svelteFsm(RootInteractionState.UNINITIALIZED, 
  emitTransitions(
    connectChildFsms(
      baseConfig, childFsmMapping
    ), rootFsmTransitions));
